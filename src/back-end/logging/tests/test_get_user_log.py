...
def test_get_user_CartCheckedOut(ip):
    ...
    y = requests.get(f"http://{ip}:3005/userlog/thomas/CartCheckedOut")
...
def test_get_user_ItemViewed(ip):
    url = f"http://{ip}:3005/userlog"
    myobj = {
                "username":"thomas",
                "userEvent":{
    ...
    y = requests.get(f"http://{ip}:3005/userlog/thomas/ItemViewed")
